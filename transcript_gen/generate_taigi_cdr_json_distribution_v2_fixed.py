...
This version generates natural plain-text transcripts first, then converts markers to JSON event scripts for later TTS/audio rendering.
...
# Hard rejects: these often indicate Qwen drifted into Cantonese, Simplified Chinese,
# or unnatural hallucinated phrasing rather than Taiwanese Taigi.
...
- 不要加標題、說明、編號、引號。
- 不要使用「台語：」或任何語言標籤。
- 不要使用粵語字詞，例如：嘅、咗、啲、係、煮緊、講緊。
- 不要使用不自然或現代廣告感詞語，例如：米其林。
...
def build_prompt(
    ...
    """Build a plain-transcript prompt.

    Important design choice: Qwen is asked to generate only natural spoken Taigi text,
    not JSON. The script converts markers to event_script afterward. This preserves
    the better language quality of the original generator while still producing JSON
    files for the audio pipeline.
    """
    style = CDR_STYLE[cdr]
    scenario = SCENARIOS[scenario_key]
    speaker_profile = speaker_profile or {}
    language_profile = language_profile or {
        "description": "台語為主，台式華語自然混入。",
        "taigi_ratio_prompt": style["taigi_ratio"],
    }
...
CDR 0 額外要求：
- 必須像正常長輩，句子自然、順序清楚。
- 若情境是診間，要清楚回答日期、早餐、家裡狀況，不要只反問。
- 不可出現明顯失智、嚴重離題、長停頓、沉默。
""" if cdr == 0 else ""
...
共同要求：
- 只輸出逐字稿正文，不要 JSON、Markdown、標題、註解、編號。
...
def safe_randint(low: int, high: int) -> int:
    """Return a random integer even when a requested range was inverted.

    This protects CDR 0 / 0.5 samples from crashing when the model emits
    [長停頓] or [沉默] even though their normal pause range is short.
    """
    low = int(low)
    high = int(high)
    if high < low:
...
def marker_to_event(marker: str, cdr: float) -> dict[str, Any]:
    ...
    if marker == "[停頓]":
        return {"type": "pause", "duration_ms": safe_randint(max(200, low), min(1200, high))}

    if marker == "[長停頓]":
        # For CDR 0 / 0.5, avoid an impossible range like 1200..700.
        if high < 1200:
            return {"type": "pause", "duration_ms": safe_randint(low, high)}
        return {"type": "pause", "duration_ms": safe_randint(max(1200, low), high)}

    if marker == "[沉默]":
        # Same safety: early CDR levels may not have a long silence range.
        if high < 1500:
            return {"type": "silence", "duration_ms": safe_randint(low, high)}
        return {"type": "silence", "duration_ms": safe_randint(max(1500, low), high)}
...
def normalize_event_script(events: Any, cdr: float) -> list[dict[str, Any]]:
    ...
    for event in events:
        ...
        try:
            duration_ms = int(float(event.get("duration_ms", safe_randint(low, high))))
        except (TypeError, ValueError):
            duration_ms = safe_randint(low, high)
...
def infer_labels_from_text(text: str) -> dict[str, Any]:
    """Lightweight heuristic labels for plain-text generation.

    These labels are not clinical truth; they are useful metadata for auditing and
    downstream feature checks. The parser/features should remain the main source.
    """
    labels = default_labels()
    labels["word_finding_count"] = sum(
        text.count(token) for token in ["想袂起", "袂記得", "毋知", "彼個", "彼个", "那個", "叫啥"]
    )
    labels["repair_count"] = sum(
        text.count(token) for token in ["毋是", "不是", "講毋著", "講錯", "應該是"]
    )
    labels["code_switch_count"] = sum(
        text.count(token) for token in ["然後", "因為", "所以", "就是", "比較", "但是"]
    )
    # Very rough repetition signal: repeated short phrase or repeated uncertainty.
    labels["repetition_count"] = len(re.findall(r"(.{1,4})[，、 ]*\1", text))
    return labels
...
def validate_sample(cdr: float, text: str, events: list[dict[str, Any]]) -> tuple[bool, str]:
    ...
    for pattern in BAD_LANGUAGE_PATTERNS:
        if pattern in text:
            return False, f"bad language artifact: {pattern}"

    if cdr == 0:
        if any(marker in text for marker in ["[長停頓]", "[沉默]", "[嘆氣]"]):
            return False, "too severe marker for CDR 0"
        # Normal cognition should be mostly answers/narrative, not only repeated questions.
        if text.count("？") >= 3 or text.count("?") >= 3:
            return False, "too many questions for CDR 0"
...
def parse_model_output(raw_response: str, cdr: float) -> tuple[str, list[dict[str, Any]], dict[str, Any], dict[str, Any]]:
    """Parse plain transcript output and convert markers to event_script.

    We intentionally avoid asking the LLM for JSON because that degraded Taigi
    naturalness. JSON/event files are created deterministically here.
    """
    ...
    labels = infer_labels_from_text(spoken_text)
    return spoken_text, events, labels, {"json_repaired_or_fallback": False, "generation_mode": "plain_text_then_event_parse"}
